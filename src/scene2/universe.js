...
export class Universe {
  ...
  async load() {
    ...
    const [grain, fig] = await Promise.all([
      loadImage('https://raw.githubusercontent.com/gireeshkumarreddy/cinematic-portofilo/main/public/tex/grain.png'),
      loadImage('https://raw.githubusercontent.com/gireeshkumarreddy/cinematic-portofilo/main/public/tools/figure.png'),
    ]);
    ...
    const imgs = await Promise.all(
      TOOLS.map((n) => loadImage(`https://raw.githubusercontent.com/gireeshkumarreddy/cinematic-portofilo/main/public/tools/${n}.png`).catch(() => null)));
    TOOLS.forEach((n, i) => {
      if (!imgs[i]) return;
      const t = texture(gl);
      upload(gl, t, imgs[i]);
